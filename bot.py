...
async def init_db():
    ...
    await db.executescript("""
        ...
        INSERT OR IGNORE INTO meta VALUES ('lobby_counter_challenge', '0');
    """)
    await db.commit()
...
def best_2v2_balance(players_mmr: dict) -> tuple:
...
async def launch_private_lobby(interaction: discord.Interaction, lobby: dict, lobby_id: str):
    ...
    embed = discord.Embed(
        ...
        description=(
            "**1.** One player creates a Custom Game in Valorant and posts the code here\n"
            "**2.** Can't decide who hosts? Use `/coinflip`\n"
            "**3.** No map decided? Use `/map` to roll one\n"
            "**4.** Play, then click **I Won** or **I Lost** below — other side confirms and MMR updates\n\n"
            "Screenshot your result if there's a dispute."
        ),
        color=discord.Color.green(),
    )
    if mode == "2v2":
...
class RematchConfirmView(discord.ui.View):
    ...
    async def confirm(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        match_embed = discord.Embed(
            ...
            description="Use `/map` to pick a map, share your lobby code here, then report the result below.",
            color=discord.Color.green(),
        )
...
@tree.command(name="map", description="Randomly pick a Skirmish map to play on")
async def map_roll(interaction: discord.Interaction):
    maps = ["Skirmish A", "Skirmish B", "Skirmish C"]
    ...
    embed.set_footer(text="Use /map again to reroll")
    await interaction.response.send_message(embed=embed)


@tree.command(name="profile", description="View a player's stats")
...
@tree.command(name="setup_spotlight", description="[Admin] Set this channel for weekly player spotlight")
...
class EndSeasonConfirmView(discord.ui.View):
    def __init__(self, admin_id: str, season: int, players: dict):
        super().__init__(timeout=60)
    ...
    @discord.ui.button(label="✅ End Season", style=discord.ButtonStyle.red)
    async def confirm(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        archive = {}
        for mode in ("1v1", "2v2"):
            top = sorted(self.players.items(), key=lambda x: x[1][f"mmr_{mode}"], reverse=True)[:10]
            archive[mode] = [{"username": p.get("username"), "mmr": p[f"mmr_{mode}"], "wins": p[f"wins_{mode}"], "losses": p[f"losses_{mode}"]} for _, p in top]
        await db.execute("INSERT INTO season_archive (season, timestamp, data) VALUES (?,?,?)",
            (self.season, datetime.now(timezone.utc).isoformat(), json.dumps(archive)))
        await db.execute("""UPDATE players SET mmr_1v1=1000, mmr_2v2=1000,
        ...
        await db.execute("UPDATE meta SET value='0' WHERE key='lobby_counter_challenge'")
        await db_set_meta("season", self.season + 1)
        await db.commit()
        await _force_refresh_leaderboards(interaction.guild)
        guild = interaction.guild
        for uid in self.players:
            member = guild.get_member(int(uid))
            if member:
                await assign_rank_role(guild, member, 1000, "1v1")
                await assign_rank_role(guild, member, 1000, "2v2")
        medals = ["🥇", "🥈", "🥉"]
        embed  = discord.Embed(title=f"Season {self.season} Complete — Season {self.season+1} Started", color=discord.Color.gold(), timestamp=datetime.now(timezone.utc))
        for mode in ("1v1", "2v2"):
            top3  = archive[mode][:3]
            lines = [f"{medals[i]} **{p['username']}** — {p['mmr']} MMR | {p['wins']}W/{p['losses']}L" for i, p in enumerate(top3)]
            embed.add_field(name=f"{mode.upper()} Top 3", value="\n".join(lines) or "No data", inline=True)
    ...
    async def on_timeout(self):
        try:
            if self.message: await self.message.edit(content="⏱️ Timed out.", embed=None, view=None)
        except Exception: pass


@tree.command(name="end_season", description="[Admin] Archive results, reset all MMR, and start next season")
async def end_season(interaction: discord.Interaction):
    if not interaction.user.guild_permissions.administrator:
        await interaction.response.send_message("❌ Admin only.", ephemeral=True); return
    season  = await db_get_season()
    players = await db_get_all_players()
    active  = sum(1 for p in players.values() if p.get("wins_1v1",0)+p.get("losses_1v1",0)+p.get("wins_2v2",0)+p.get("losses_2v2",0) > 0)
    embed   = discord.Embed(
        title=f"End Season {season}?",
        description=(
            f"This will:\n"
            f"• Archive top 10 for 1v1 and 2v2\n"
            f"• Reset all **{active}** active players to 1000 MMR\n"
            f"• Clear match history and H2H records\n"
            f"• Start Season {season + 1}\n\n"
            f"**This cannot be undone.**"
        ),
        color=discord.Color.orange(),
    )
    view = EndSeasonConfirmView(str(interaction.user.id), season, players)
    await interaction.response.send_message(embed=embed, view=view, ephemeral=True)
    view.message = await interaction.original_response()


@tree.command(name="season_history", description="View archived season results")
@app_commands.describe(season_num="Season number to view (leave blank for all)")
async def season_history(interaction: discord.Interaction, season_num: int = None):
    async with db.execute("SELECT season, timestamp, data FROM season_archive ORDER BY season DESC") as cur:
        rows = await cur.fetchall()
    if not rows:
        await interaction.response.send_message("No season history yet.", ephemeral=True); return
    if season_num:
        rows = [r for r in rows if r[0] == season_num]
        if not rows:
            await interaction.response.send_message(f"No data for Season {season_num}.", ephemeral=True); return
    medals = ["🥇", "🥈", "🥉"]
    embed  = discord.Embed(title="Season History", color=discord.Color.gold())
    for row in rows[:5]:
        season_n, ts, data_str = row[0], row[1], row[2]
        data = json.loads(data_str)
        date = ts[:10]
        for mode in ("1v1", "2v2"):
            top3  = data.get(mode, [])[:3]
            lines = [f"{medals[i]} **{p['username']}** — {p['mmr']} MMR" for i, p in enumerate(top3)]
            embed.add_field(name=f"Season {season_n} {mode.upper()} ({date})", value="\n".join(lines) or "No data", inline=True)
    if len(rows) > 5:
        embed.set_footer(text=f"Showing 5 of {len(rows)} seasons. Use /season_history [number] for a specific one.")
    await interaction.response.send_message(embed=embed)


@tree.command(name="set_season", description="[Admin] Manually set the current season number")
@app_commands.describe(number="Season number to set")
async def set_season(interaction: discord.Interaction, number: int):
    if not interaction.user.guild_permissions.administrator:
        await interaction.response.send_message("❌ Admin only.", ephemeral=True); return
    if number < 1:
        await interaction.response.send_message("❌ Must be 1 or higher.", ephemeral=True); return
    await db_set_meta("season", number)
    await db.commit()
    await _force_refresh_leaderboards(interaction.guild)
    await interaction.response.send_message(f"✅ Season set to **{number}**. Leaderboards updated.", ephemeral=True)


async def _force_refresh_leaderboards(guild: discord.Guild):
    """Immediately push updated leaderboard embeds to the pinned messages."""
    cfg     = load_config()
    players = await db_get_all_players()
    season  = await db_get_season()
    for mode in ("1v1", "2v2"):
...
class WipeConfirmView(discord.ui.View):
    ...
    async def confirm(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        embed = discord.Embed(
            ...
            description="All players, MMR, history, lobbies, bans, and season data deleted. Leaderboards reset. Counters at 0.",
            color=discord.Color.red(),
            timestamp=datetime.now(timezone.utc),
        )
...
@tree.command(name="leavelobby", description="Unstick yourself if you're stuck in a deleted lobby")
...
async def setup_commands(interaction: discord.Interaction):
    ...
    embed.add_field(
        ...
        value=(
            ...
            "`/map` — Roll a random Skirmish map (A, B, or C)"
        ),
        inline=False,
    )
    embed.add_field(
    ...
    embed.add_field(
        ...
        value=(
            ...
            "`/season_history` — View past season results"
        ),
        inline=False,
    )
    embed.add_field(
...
async def help_cmd(interaction: discord.Interaction):
    ...
    embed.add_field(name="Lobbies",  value="`/create_lobby` · `/challenge @user` · `/coinflip` · `/map`", inline=False)
